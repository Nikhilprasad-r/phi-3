...
export async function generateTopics(language: string, years: number) {
  const promptContent = `You are tasked with generating 3 interview topics for a software development role. The candidate has ${years} years of experience in ${language}. 
  ...
  const generateTopicsPrompt = [
    {
      role: "system",
      content: `You are a programming expert and interview question generator. ${promptContent}`,
    },
    {
      role: "user",
